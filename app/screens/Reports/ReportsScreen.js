...
function HomeScreen({ navigation, route }) {
		...
		filteredItems.map((items) => {
			items.map((item) => {
				if (lineChartTimeframe == "daily") {
					...
					tempLineDataDaily[tempDate.getDay()] += item.totalValue;
				} else if (lineChartTimeframe == "weekly") {
	...
	const timeframeLabels = () => {
		if (lineChartTimeframe == "daily") {
			...
			return [
				week[0].split("-")[1] + "-" + week[0].split("-")[2],
				week[1].split("-")[1] + "-" + week[1].split("-")[2],
				week[2].split("-")[1] + "-" + week[2].split("-")[2],
				week[3].split("-")[1] + "-" + week[3].split("-")[2],
				week[4].split("-")[1] + "-" + week[4].split("-")[2],
				week[5].split("-")[1] + "-" + week[5].split("-")[2],
				week[6].split("-")[1] + "-" + week[6].split("-")[2],
			];
		} else if (lineChartTimeframe == "weekly") {
			const curr = new Date();
